...
/**
 * ProtectRegistrationStaff: Ensures only registration_staff users can access a page.
 * Redirects all other roles to /login.
 * Stores event_id in sessionStorage for component usage.
 */
export function ProtectRegistrationStaff({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // Check if user is registration_staff
        if (userData.role !== 'registration_staff') {
          router.push('/login');
          return;
        }
