...
/**
 * ProtectCateringStaff: Ensures only catering_staff users can access a page.
 * Redirects all other roles to /login.
 * Stores event_id in sessionStorage for component usage.
 */
export function ProtectCateringStaff({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // Check if user is catering_staff
        if (userData.role !== 'catering_staff') {
          router.push('/login');
          return;
        }
